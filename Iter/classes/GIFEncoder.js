	...
	this.getData = function() {
		...
		for(var p = 0; p < this.pages.length; p++) {
			for(var i = 0; i < ByteArray.pageSize; i++) {
				tArr.push(this.pages[p][i]);
				if(p >= (this.pages.length - 1) && i >= (this.cursor - 1)) break;
			}
		}
		
		return new Uint8Array(tArr);
	}
...
function GIFEncoder(width, height) {
	...
	this.setDispose = function(disposalCode) {
	...
	this.writeGraphicCtrlExt = function() {
	...
	this.writeNetscapeExt = function() {
		this.out.writeByte(0x21); // extension introducer
	...
	//Writes color table
	this.writePalette = function() {
		this.out.writeBytes(this.colorTab);
		var n = (3 * 256) - this.colorTab.length;
		for(var i = 0; i < n; i++) this.out.writeByte(0);
	};
	
	this.writeShort = function(pValue) {
